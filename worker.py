...
def sync_strava_activities(conn):
    ...
    Version avec une gestion d'erreur ciblée sur l'authentification.
    """
    print("Lancement du cycle de synchronisation Strava...")
    authed_client = None # On initialise le client à None

    try:
        print("Tentative d'authentification Strava...")
        client = Client()
        # --- On isole l'appel qui pose problème ---
        try:
            ...
            return # On arrête la fonction si l'authentification échoue
        # -----------------------------------------
        ...
        print("Une erreur est survenue durant la synchronisation des activités Strava (après l'authentification) :")
        print(traceback.format_exc())
